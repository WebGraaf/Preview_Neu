...
export const SplitMediaTextInvert: React.FC<SplitMediaTextInvertProps> = ({
  ...
  useEffect(() => {
    ...
    // Container slide in - removed 3D rotationY for better performance
    tl.fromTo(
      container,
      { opacity: 0, y: 50 },
    ...
    // Image slide in - removed blur filter (not GPU-accelerated), reduced distance
    if (image) {
    ...
    // Text content staggered reveal - simplified, removed scaleX
    if (text?.children) {
      tl.fromTo(
        text.children,
        { opacity: 0, y: 20 },
